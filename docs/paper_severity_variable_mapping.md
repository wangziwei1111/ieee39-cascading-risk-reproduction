...
| `P_li(n)` | 第 n 条线路有功功率标幺值 | LFOR | `results/tables/markov_line_flow_details.csv` | `P_li_pu` | true | 基于 `max(abs(PF),abs(PT))/RATE_A` |
| `P_li_max(n)` | 第 n 条线路最大有功功率标幺值 | LFOR | `results/tables/markov_line_flow_details.csv` | `P_li_max_pu` | true | 当前设为1；`RATE_A`作为有功上限近似，待校准 |
| `sev_ev_lfor(E_k)` | 第 k 级线路越限严重度 | LFOR | `results/tables/markov_line_flow_details.csv` | `line_severity_component` 按stage求和 | true | 无 |
| `R_LFOR` | 单条事故链线路越限风险值 | VaR SLFOR | `results/tables/markov_risk_samples_paper_severity.csv` | `paper_LFOR` | true | 当前为line-only概率近似 |
| `U_m` | 第 m 个节点电压标幺值 | NVOR | `results/tables/markov_bus_voltage_details.csv` | `voltage_pu` | true | 无 |
| `sev_ev_nvor(E_k)` | 第 k 级节点电压越限严重度 | NVOR | `results/tables/markov_bus_voltage_details.csv` | `voltage_severity_component` 按stage求和 | true | 无 |
