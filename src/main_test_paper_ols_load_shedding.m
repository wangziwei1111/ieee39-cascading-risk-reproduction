...
for k = 1:numel(test_cases)
    ...
    comparison_rows{end + 1, 1} = table( ... %#ok<AGROW>
        ...
        logical(pf_result_success(pf_before)), logical(shed_simple.converged_after_shed), logical(shed_ols.converged_after_shed), ...
...
for i = 1:height(comparison)
    fprintf(fid, '%s: simple=%.6f MW, ols=%.6f MW, status=%s, note=%s\n', ...
        comparison.case_id{i}, comparison.simple_total_load_shed_mw(i), ...
        comparison.ols_total_load_shed_mw(i), comparison.ols_status{i}, comparison.note{i});
end
end
