...
rows = add_row(rows, "wind_power_curve_matches_paper", all(curve.match_status=="match"), ...
    ...
    "Review whether wind-speed calibration aliases should use paper cut-in/cut-out 2/20 instead of scenario default 3/25.", ...
    "This is diagnostic only; no parameter change made.");
...
rows = add_row(rows, "wind_trip_probability_available", any(trip.wind_trip_probability_available), ...
    ...
    "If paper wind-speed risk depends on renewable trip probability, enable/record a calibrated wind-trip diagnostic before refinement.", ...
...
rows = add_row(rows, "severity_distribution_direction", all(cri_sev.direction_match), ...
    ...
    "Inspect stage severity and wind dispatch if severity-only risk is opposite to paper.", "Paper direction expects 12.00 lower than 11.28.");
...
rows = add_row(rows, "chain_probability_distribution_direction", all(cri_full.direction_match), ...
    ...
    "Inspect chain transition probabilities and candidate selection if full-event risk is opposite to paper.", "Uses existing 10-trial pilot.");
...
rows = add_row(rows, "paper_direction_vs_sim_direction", all(cri_full.direction_match), ...
    ...
    "Do not enter parameter refinement until wind-speed direction is explained.", "This is the immediate go/no-go issue.");
...
elseif any(rows.diagnosis_item=="wind_trip_probability_available" & rows.status=="fail")
    cause = "wind_trip_model_missing"; fix = "implement_or_record_wind_trip_model";
