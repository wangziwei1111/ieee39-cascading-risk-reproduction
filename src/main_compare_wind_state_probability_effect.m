function main_compare_wind_state_probability_effect()
%MAIN_COMPARE_WIND_STATE_PROBABILITY_EFFECT Offline diagnostic P_line * P_wt comparison.
project_root = fileparts(fileparts(mfilename('fullpath')));
addpath(fullfile(project_root, 'config'));
addpath(genpath(fullfile(project_root, 'src')));

case_dir = fullfile(project_root, 'results', 'renewable', 'wind_state_probability_diagnostic_smoke', ...
    'diagnostic_linear_voltage_probability');
records_path = fullfile(case_dir, 'markov_chain_records.mat');
if ~exist(records_path, 'file')
...
wind_state_table = readtable(fullfile(case_dir, 'wind_state_probability_stage_details.csv'), 'TextType', 'string');
comparison = build_comparison(stage_probability_table, line_flow_detail_table, bus_voltage_detail_table, wind_state_table, cfg);
summary = build_summary(comparison);

out_dir = fullfile(project_root, 'results', 'renewable');
if ~exist(out_dir, 'dir'), mkdir(out_dir); end
writetable(comparison, fullfile(out_dir, 'wind_state_probability_effect_comparison.csv'));
writetable(summary, fullfile(out_dir, 'wind_state_probability_effect_summary.csv'));
fprintf('wind state probability effect comparison written.\n');
end

function comparison = build_comparison(stage_probability_table, line_flow_detail_table, bus_voltage_detail_table, wind_state_table, cfg)
rows = {};
for i = 1:height(stage_probability_table)
    key = stage_probability_table(i, {'initial_branch', 'trial_id', 'stage_id'});
    wmask = wind_state_table.initial_branch == key.initial_branch & ...
        wind_state_table.trial_id == key.trial_id & wind_state_table.stage_id == key.stage_id;
    if any(wmask)
        p_wt = wind_state_table.p_wt_Ek(find(wmask, 1));
    else
        p_wt = NaN;
    end
    ...
    rows{end+1,1} = table(key.initial_branch, key.trial_id, key.stage_id, ...
        p_line, p_wt, p_line * p_wt, llr, lfor, nvor, cri, p_line - p_line * p_wt, ...
        "diagnostic only; P_wt not integrated into formal paper_formula", ...
        'VariableNames', {'initial_branch', 'trial_id', 'stage_id', 'P_line_Ek', ...
        'P_wt_Ek', 'P_line_times_P_wt', 'severity_LLR', 'severity_LFOR', ...
