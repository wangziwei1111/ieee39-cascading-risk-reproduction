...
for k = 1:numel(scenario_ids)
    ...
    wind_trip_metrics = read_wind_trip_metrics(table_dir);
    ...
    rows{end + 1, 1} = table(string(scenario_id), scenario.total_wind_capacity_mw, ...
        ...
        basecase_metrics.base_voltage_violation_count, wind_trip_metrics.enabled, ...
        wind_trip_metrics.total_rows, wind_trip_metrics.max_trip_probability, ...
        wind_trip_metrics.p95_trip_probability, wind_trip_metrics.num_probability_positive, ...
        read_basecase_status(table_dir), ...
        ...
        'basecase_voltage_violation_count', 'wind_trip_record_enabled', ...
        'wind_trip_detail_rows', 'max_wind_trip_probability', 'p95_wind_trip_probability', ...
        'num_wind_trip_probability_positive', 'basecase_converged', 'chain_count', 'invalid_stage_ratio', ...
...
function metrics = read_wind_trip_metrics(table_dir)
path = fullfile(table_dir, 'wind_trip_probability_summary.csv');
metrics = struct('enabled', false, 'total_rows', 0, 'max_trip_probability', NaN, ...
    'p95_trip_probability', NaN, 'num_probability_positive', 0);
if ~exist(path, 'file')
    return;
end
tbl = readtable(path);
metrics.enabled = true;
metrics.total_rows = get_scalar(tbl, 'total_rows');
metrics.max_trip_probability = get_scalar(tbl, 'max_trip_probability');
metrics.p95_trip_probability = get_scalar(tbl, 'p95_trip_probability');
metrics.num_probability_positive = get_scalar(tbl, 'num_probability_positive');
end
