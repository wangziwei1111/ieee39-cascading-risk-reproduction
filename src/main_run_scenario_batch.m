...
scenario_result = struct('scenario_id', string(scenario_id), ...
    ...
    'wind_trip_record_enabled', exist(fullfile(scenario_dir, 'tables', 'wind_trip_probability_summary.csv'), 'file') == 2, ...
    'wind_trip_detail_rows', read_wind_trip_summary_value(scenario_dir, 'total_rows', 0), ...
    'max_wind_trip_probability', read_wind_trip_summary_value(scenario_dir, 'max_trip_probability', NaN), ...
    'p95_wind_trip_probability', read_wind_trip_summary_value(scenario_dir, 'p95_trip_probability', NaN), ...
    'num_wind_trip_probability_positive', read_wind_trip_summary_value(scenario_dir, 'num_probability_positive', 0), ...
...
function value = read_wind_trip_summary_value(scenario_dir, field_name, default_value)
path = fullfile(scenario_dir, 'tables', 'wind_trip_probability_summary.csv');
value = default_value;
if ~exist(path, 'file')
    return;
end
