function main_score_full_event_formal_var_alignment()
%MAIN_SCORE_FULL_EVENT_FORMAL_VAR_ALIGNMENT Score trend and scale alignment.
project_root = fileparts(fileparts(mfilename('fullpath')));
out_root = fullfile(project_root, 'results', 'calibration', 'full_event_formal_var_pilot');
gap = readtable(fullfile(out_root, 'full_event_formal_var_to_paper_gap.csv'), 'TextType', 'string');
...
for p = 1:numel(parameter_sets)
    ...
    rows{end+1,1} = table(parameter_sets(p), height(D), mean_gap, med_gap, ...
        trend(1), trend(2), trend(3), trend_rate, NaN, ...
        classify_recommendation(trend_rate, med_gap, height(D)), ...
...
rank = NaN(height(summary), 1);
rank(order) = (1:height(summary)).';
summary.score_rank = rank;
writetable(summary, fullfile(out_root, 'full_event_formal_var_score_summary.csv'));
fprintf('full-event score summary written: %s\n', out_root);
end

function tf = topology_match(D)
tf = compare_pair(D, "concentrated_bus34", "distributed_30_39", "CRI_recomputed", true);
end

function tf = wind_speed_match(D)
tf = compare_pair(D, "wind_speed_11_28", "wind_speed_12_00", "CRI_recomputed", false);
end
...
if any(isnan(vals)) || any(isnan(papers))
    tf = NaN;
else
    tf = double(sign(vals(3)-vals(1)) == sign(papers(3)-papers(1)));
end
end

function tf = compare_pair(D, a, b, metric, lower_expected)
vals = scenario_values(D, [a,b], metric);
papers = paper_values(D, [a,b], metric);
if any(isnan(vals)) || any(isnan(papers))
    tf = NaN;
else
    if lower_expected
        tf = double((vals(2) < vals(1)) == (papers(2) < papers(1)));
    else
        tf = double(sign(vals(2)-vals(1)) == sign(papers(2)-papers(1)));
    end
end
end
...
for i = 1:numel(scenarios)
    row = D(D.scenario_id == scenarios(i) & D.metric_name == metric, :);
    if ~isempty(row), vals(i) = row.sim_var_value(1); end
end
end
...
for i = 1:numel(scenarios)
    row = D(D.scenario_id == scenarios(i) & D.metric_name == metric, :);
    if ~isempty(row), vals(i) = row.paper_value(1); end
end
end
