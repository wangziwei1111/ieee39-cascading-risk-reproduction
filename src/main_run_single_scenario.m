...
try
    ...
    scenario_result = build_scenario_result(scenario_id, scenario, cfg, renewable_info, basecase_converged, 'success', '');
    fprintf('场景运行完成：%s\n', scenario_id);
catch ME
...
function scenario_result = build_scenario_result(scenario_id, scenario, cfg, renewable_info, basecase_converged, status, note)
...
basecase_path = fullfile(cfg.results_table_dir, 'basecase_validation.csv');
...
basecase_table = readtable(basecase_path);
total_wind_output_mw = get_scalar_or_nan(basecase_table, 'total_wind_output_mw');
if isfield(renewable_info, 'total_wind_output_mw')
    total_wind_output_mw = renewable_info.total_wind_output_mw;
end
...
scenario_result = struct('scenario_id', string(scenario_id), ...
    ...
    'total_wind_output_mw', total_wind_output_mw, ...
