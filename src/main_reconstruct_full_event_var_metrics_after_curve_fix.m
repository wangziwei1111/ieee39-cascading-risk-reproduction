function main_reconstruct_full_event_var_metrics_after_curve_fix()
...
for p = 1:numel(parameter_sets)
    for s = 1:numel(scenario_ids)
        source_file = fullfile(out_root, parameter_sets(p), scenario_ids(s), 'tables', 'markov_chain_summary.csv');
        if exist(source_file, 'file') ~= 2, continue; end
        T = readtable(source_file, 'TextType', 'string', 'Delimiter', ',');
        S = readtable(fullfile(out_root, parameter_sets(p), scenario_ids(s), 'scenario_config_snapshot.csv'), 'TextType', 'string', 'Delimiter', ',');
            ...
            for sg = 1:numel(sigmas)
                for m = 1:numel(metrics)
                    ...
                    rows{end+1,1} = table(parameter_sets(p), scenario_ids(s), variants(v), sigmas(sg), metrics(m), ...
                        q(values(valid), sigmas(sg)), height(T), sum(valid), summarize_status(T), summarize_stage_mode(T), ...
                        string(S.wind_power_curve_profile(1)), "empirical_quantile", string(source_file), ...
                        "After-curve-fix full-event metric reconstruction; not final benchmark.", ...
                        ...
                        'stage_probability_mode','wind_power_curve_profile','quantile_rule','source_file','note'}); %#ok<AGROW>
                end
            end
        end
    end
...
writetable(metrics_tbl, fullfile(out_root, 'full_event_var_metrics_after_curve_fix.csv'));
end
...
risk.full_event_chain_risk_display.CRI_recomputed = 0.6*risk.full_event_chain_risk_display.SLLR + 0.2*risk.full_event_chain_risk_display.SLFOR + 0.2*risk.full_event_chain_risk_display.SNVOR;
...
if ismember('chain_probability_status', T.Properties.VariableNames), s = strjoin(unique(string(T.chain_probability_status), 'stable'), ';'); else, s = "missing"; end
end

function s = summarize_stage_mode(T)
if ismember('stage_probability_mode', T.Properties.VariableNames), s = strjoin(unique(string(T.stage_probability_mode), 'stable'), ';'); else, s = "missing"; end
end
