function main_run_transition_probability_full_event_trace_smoke()
%MAIN_RUN_TRANSITION_PROBABILITY_FULL_EVENT_TRACE_SMOKE Tiny Markov smoke using Bernoulli full-event probability recording.
project_root = fileparts(fileparts(mfilename('fullpath')));
addpath(fullfile(project_root, 'config'));
addpath(genpath(fullfile(project_root, 'src')));

out_dir = fullfile(project_root, 'results', 'calibration', 'transition_probability', 'full_event_trace_smoke');
ensure_dir(out_dir);
...
stage_transition_details = flatten_stage_transition_probability_records_local(chain_records);
...
write_log(fullfile(out_dir, 'transition_probability_trace_log.txt'), cfg, scenario, chain_summary, stage_transition_details, candidate_trace);
fprintf('full-event transition probability trace smoke written: %s\n', out_dir);
end
...
for i = 1:numel(chain_records)
    ...
    for s = 1:numel(c.stage_records)
        ...
        rows{end + 1, 1} = table( ... %#ok<AGROW>
            ...
            string(get_stage_field(st, 'terminated_reason', "")), ...
            ...
            logical(get_detail_field(d, 'terminal_stage_flag', false)), ...
            logical(get_detail_field(d, 'should_multiply_candidate_complements', true)), ...
            string(resolve_candidate_selection_consistency_status(d)), ...
            ...
            'probability_status', 'terminal_stage_flag', ...
            'should_multiply_candidate_complements', 'candidate_selection_consistency_status', ...
            'missing_reason', 'selected_branch_ids', ...
...
fprintf(fid, 'transition_probability_full_event_trace_smoke\n');
...
fprintf(fid, 'note=Small diagnostic smoke only; full-event probability changes recording, not sampling or candidate selection.\n');
end
...
function status = resolve_candidate_selection_consistency_status(d)
status = "not_checked";
if isstruct(d) && isfield(d, 'probability_status')
    if string(d.probability_status) == "inconsistent_candidate_selection" || ...
            string(d.probability_status) == "inconsistent_random_selection"
        status = string(d.probability_status);
    elseif string(d.probability_status) == "full_event_available" || ...
            string(d.probability_status) == "terminal_stage_probability_one"
        status = "consistent_or_excluded_terminal";
    end
end
end
