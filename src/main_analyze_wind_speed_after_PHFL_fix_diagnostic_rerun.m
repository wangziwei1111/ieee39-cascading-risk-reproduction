function main_analyze_wind_speed_after_PHFL_fix_diagnostic_rerun()
...
var_path = fullfile(out_dir, 'wind_speed_after_PHFL_fix_var_metrics.csv');
cmp_path = fullfile(out_dir, 'wind_speed_before_after_PHFL_fix_comparison.csv');
if exist(fullfile(rerun_root, 'high_hidden_failure'), 'dir') ~= 7
    V = table(strings(0,1), strings(0,1), zeros(0,1), strings(0,1), zeros(0,1), zeros(0,1), zeros(0,1), strings(0,1), strings(0,1), ...
        ...
        'hidden_failure_loading_probability_mode','note'});
    writetable(V, var_path);
    before_path = fullfile(before_root, 'wind_speed_component_diagnostic_var_metrics.csv');
    if exist(before_path, 'file') == 2
        B = readtable(before_path, 'TextType','string');
...
error('Actual after-P_HF_L-fix rerun analysis is not implemented because rerun was not expected unless issue confirmed.');
end
    ...
    for m = 1:numel(metrics)
        for s = 1:numel(sigmas)
            ...
            rows{end+1,1}=table(psets(p),metrics(m),sigmas(s),a.var_value(1),b.var_value(1),before_dir,NaN,NaN,"skipped_no_issue_confirmed",false, ...
                "12mps_expected_lower_or_not_higher", before_dir=="matches_paper_direction", false, "PHFL_fix_not_run_no_issue_confirmed", ...
                "After-fix rerun skipped because below-Lmax P_HF_L issue was not confirmed.", ...
...
if isempty(rows), C = empty_comparison(); else, C = vertcat(rows{:}); end
end

function C = empty_comparison()
C = table(strings(0,1), strings(0,1), zeros(0,1), zeros(0,1), zeros(0,1), strings(0,1), zeros(0,1), zeros(0,1), ...
    strings(0,1), false(0,1), strings(0,1), false(0,1), false(0,1), strings(0,1), strings(0,1), ...
    'VariableNames', {'parameter_set_id','metric_name','sigma','before_11_28_var','before_12_00_var','before_direction', ...
    'after_11_28_var','after_12_00_var','after_direction','direction_changed','paper_expected_direction','before_match', ...
    'after_match','interpretation','note'});
end
