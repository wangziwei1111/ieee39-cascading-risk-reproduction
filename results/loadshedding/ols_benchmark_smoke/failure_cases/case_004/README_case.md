# OLS Failure Case case_004

- scenario_id: `distributed_wind_3000mw_base`
- initial_branch: 10
- trial_id: 2
- stage_id: 3
- failure_type: `pf_after_apply_nonconverged`
- trigger_reason: `nonconverged_powerflow`
- why_selected: representative OPF success but PF after apply failed

This case was reconstructed from existing OLS benchmark smoke `chain_records`. It does not rerun Markov sampling.

Replay files include `mpc_before_ols.mat`, `mpc_opf_with_shed_generators.mat`, `opf_result.mat`, `mpc_after_apply_load_only.mat`, `runpf_after_apply_result.mat`, and `ols_detail.mat`.

Recorded replay status: opf_success=1, pf_success_after_apply=0, message=OLS OPF succeeded, but AC PF did not converge after applying load shedding.
